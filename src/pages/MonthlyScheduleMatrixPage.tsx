...
export default function MonthlyScheduleMatrixPage({
  ...
  const [weekStartDate, setWeekStartDate] = useState(() => {
    ...
    return toDateInputValue(
      getThursdayStart(requestedDate ? parseLocalDate(requestedDate) : new Date())
    );
  });
  ...
  async function saveCell(data: {
    ...
    const resident = residents.find((item) => item.id === data.personId);
    if (!resident) return;
    ...
    const validation = validateCallAssignment({
      ...
      blocks,
      blockAssignments,
      allowCoverageOverride: Boolean(data.notes.trim()),
    });

    if (!validation.allowed) {
      setActionError(ruleIssueSummary(validation) || "This assignment is not allowed.");
      return;
    }

    if (validation.requiresOverride && !data.notes.trim()) {
      setActionError("Enter an override reason before saving this assignment.");
      return;
    }
  ...
  async function handleImport(file: File) {
    ...
    setSelectedImportIndexes(
      rows
        .map((row, index) => (row.action === "safe-new" ? index : -1))
        .filter((index) => index >= 0)
    );
  }

  async function applyImport() {
    const selected = new Set(selectedImportIndexes);
    const chosenRows = importRows.filter((row, index) => selected.has(index) && row.cell);
    const cells = chosenRows.map((row) => row.cell!);
    const replaceDifferent = chosenRows.some(
      (row) => row.action === "replacement" || row.action === "conflict"
    );
  ...
  return (
    <Box sx={{ width: "100%", maxWidth: "none", minWidth: 0 }}>
        ...
        <Stack direction="row" spacing={0.75} flexWrap="wrap" useFlexGap>
          ...
          {allowBuild && (
            <>
              ...
              <Button variant={allPublished ? "outlined" : "contained"} color={allPublished ? "warning" : "primary"} onClick={() => void handlePublishToggle()} sx={{ textTransform: "none", fontWeight: 850 }}>
      ...
      {message && <Alert severity="success" sx={compactAlertSx} onClose={() => setMessage("")}>{message}</Alert>}
      {actionError && <Alert severity="error" sx={compactAlertSx} onClose={() => setActionError("")}>{actionError}</Alert>}
      {error && <Alert severity="error" sx={compactAlertSx}>{error}</Alert>}
      {!allowBuild && !allPublished && <Alert severity="info" sx={compactAlertSx}>No published schedule is available for this period.</Alert>}
                ...
                {RESIDENT_CALL_SERVICES.map((service) => (
                  <Box key={service.id} sx={{ display: "contents" }}>
                    ...
                    {days.map((date) => {
                      ...
                      const critical = allowBuild && issues.some((issue) => issue.severity === "critical");
                      const warning = allowBuild && issues.some((issue) => issue.severity === "warning");
                      return (
                        <Box key={`${service.id}-${date}`} onClick={() => { if (allowBuild && available) setEditingCell({ date, service }); }} sx={{ ...matrixCell, cursor: allowBuild && available ? "pointer" : "default", backgroundColor: !available ? "#f1f5f9" : critical ? "#fff1f2" : warning ? "#fffbeb" : auto ? "#f5f3ff" : isHospitalHoliday(date) ? "#fff7ed" : "white", opacity: available ? 1 : 0.65 }}>
                          {cell ? (
                            <Stack spacing={0.15}>
                              ...
                              {allowBuild && manual && <Button size="small" color="error" onClick={(event) => { event.stopPropagation(); void handleRemoveCell(date, service); }} sx={{ minWidth: 0, width: "fit-content", p: "0 3px", fontSize: 9, textTransform: "none" }}>Clear</Button>}
                            </Stack>
                          ) : <Typography fontSize={10} color="text.secondary">{!available ? "Weekend/holiday only" : allowBuild ? "Assign" : "—"}</Typography>}
                        </Box>
                      );
                    })}
                  </Box>
      ...
      <CallImportDialog
        ...
        onSelectedIndexes={setSelectedImportIndexes}
        onCancel={() => { setImportRows([]); setSelectedImportIndexes([]); }}
        onApply={applyImport}
      />
...
function CallImportDialog({
  ...
  onSelectedIndexes: (value: number[]) => void;
  onCancel: () => void;
  ...
  function toggle(index: number) {
    ...
    if (!row.cell || ["same", "invalid", "unmatched"].includes(row.action)) return;
    const next = new Set(selected);
    if (next.has(index)) next.delete(index);
    else next.add(index);
    onSelectedIndexes(Array.from(next).sort((a, b) => a - b));
  }

  return (
    <Dialog open={open} onClose={onCancel} fullWidth maxWidth="lg">
      <DialogTitle>Smart Call Schedule Import</DialogTitle>
      <DialogContent>
        <Alert severity="info" sx={{ mb: 1 }}>
          Safe new assignments are selected automatically. Replacements and conflicts require your explicit selection. Invalid and unmatched rows cannot be imported.
        </Alert>
        ...
        <Box sx={{ maxHeight: 470, overflow: "auto", border: "1px solid", borderColor: "divider", borderRadius: 2 }}>
          {rows.slice(0, 700).map((row, index) => {
            const selectable = Boolean(row.cell) && !["same", "invalid", "unmatched"].includes(row.action);
            return (
              <Box
                key={`${row.sourceDate}-${row.sourceService}-${index}`}
                sx={{
                  display: "grid",
                  gridTemplateColumns: "32px .8fr 1fr 1fr .7fr 2fr",
                  gap: 1,
                  alignItems: "center",
                  p: 0.7,
                  borderBottom: "1px solid #eef2f7",
                  backgroundColor: row.action === "conflict" ? "#fffbeb" : row.action === "invalid" || row.action === "unmatched" ? "#fff1f2" : "white",
                }}
              >
                <Checkbox size="small" checked={selected.has(index)} disabled={!selectable} onChange={() => toggle(index)} />
                <Typography fontSize={11.5}>{row.sourceDate}</Typography>
                <Typography fontSize={11.5} fontWeight={800}>{row.sourceService}</Typography>
                <Typography fontSize={11.5}>{row.sourceResident}</Typography>
                <Chip label={row.action} size="small" sx={{ height: 20, fontSize: 9.5, width: "fit-content" }} />
                <Typography fontSize={11} color={["conflict", "invalid", "unmatched"].includes(row.action) ? "error" : "text.secondary"}>{row.message}</Typography>
              </Box>
            );
          })}
        </Box>
      ...
      <DialogActions>
        <Button onClick={onCancel}>Cancel</Button>
        <Button variant="contained" disabled={selectedIndexes.length === 0} onClick={onApply}>
          Import {selectedIndexes.length} Selected as Draft
        </Button>
      </DialogActions>
    </Dialog>
  );
