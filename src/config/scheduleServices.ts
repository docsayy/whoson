...
export function getResidentCallServicesForDate(date: string) {
  const usesWeekendSchedule =
    isWeekendScheduleDate(date) || isHospitalHoliday(date);
  const ids = usesWeekendSchedule ? WEEKEND_SERVICE_IDS : WEEKDAY_SERVICE_IDS;
...
export function isServiceAvailableOnDate(
  ...
  return (
    !definition.weekendOnly ||
    isWeekendScheduleDate(date) ||
    isHospitalHoliday(date)
  );
}
