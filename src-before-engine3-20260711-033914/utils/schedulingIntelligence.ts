...
import { isShortDutyService } from "../config/scheduleServices";
import {
...
function withIssueDate(date: string, message: string) {
  return `${formatIssueDate(date)}: ${message}`;
}
...
function isAllowedWeekendContinuousCoverage(
  date: string,
...
function allPairsAreAllowed(
  date: string,
  serviceIds: string[]
) {
  for (let first = 0; first < serviceIds.length; first += 1) {
    for (let second = first + 1; second < serviceIds.length; second += 1) {
      if (!isAllowedWeekendContinuousCoverage(date, serviceIds[first], serviceIds[second])) {
        return false;
      }
    }
  }
...
export function detectDailyScheduleIssues({
  ...
  for (const item of effectiveCells) {
    ...
    if (resident && !residentMeetsServiceRequirement(resident, item.service)) {
      issues.push({
        ...
        message: withIssueDate(
          date,
          `${resident.displayName} is ${resident.pgy}, but ${item.service.name} requires ${getRequiredTrainingText(item.service)}.`
        ),
      });
    }
    ...
    if (item.manual && item.auto && item.manual.residentId !== item.auto.residentId) {
      issues.push({
        ...
        message: withIssueDate(
          date,
          `${item.service.name} was manually changed from ${item.auto.residentName} to ${item.manual.residentName}.`
        ),
      });
    }
  }
  ...
  for (const [residentId, residentCells] of cellsByResident.entries()) {
    ...
    const serviceIds = residentCells.map((item) => item.service.id);
    const overlapsAllowed = allPairsAreAllowed(date, serviceIds);

    if (residentCells.length > 1 && !overlapsAllowed) {
      issues.push({
        ...
        message: withIssueDate(
          date,
          `${resident.displayName} is assigned to ${residentCells
    ...
    const hasShortDuty = residentCells.some((item) =>
      isShortDutyService(item.service)
    );

    if (hasNightFloat && hasShortDuty) {
      issues.push({
        id: `nf-short-duty-${date}-${residentId}`,
        severity: "critical",
        category: "short-duty-conflict",
        date,
        residentId,
        residentName: resident.displayName,
        title: "Night Float and short duty conflict",
        message: withIssueDate(
          date,
          `${resident.displayName} cannot be assigned to Night Float and short duty on the same date.`
        ),
      });
    }

    if (hasNightFloat && hasDayService && !hasShortDuty && !overlapsAllowed) {
      issues.push({
        ...
        message: withIssueDate(
          date,
          `${resident.displayName} is assigned to night float and a daytime call/service on the same date.`
        ),
      });
    }
  }
    ...
    for (const item of effectiveCells) {
      ...
      if (vacationAssignment) {
        issues.push({
          ...
          message: withIssueDate(
            date,
            `${cell.residentName} is on ${vacationAssignment.rotationName} but assigned to ${item.service.name}.`
          ),
        });
      }
      ...
      const isChiefOnCall = item.service.id === "chief-on-call";

      if (
        jeopardyAssignment &&
        !isChiefOnCall &&
        !normalize(item.service.name).includes("jeopardy")
      ) {
        issues.push({
          ...
          message: withIssueDate(
            date,
            `${cell.residentName} is assigned to Jeopardy in the block schedule but also assigned to ${item.service.name}.`
          ),
        });
      }
    }
