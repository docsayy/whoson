  ...
  return (
    <Box sx={{ width: "100%", maxWidth: "none", minWidth: 0 }}>
      ...
      {!allowBuild && publishedAssignments.length === 0 && (
        <Alert severity="warning" sx={compactAlertSx}>No published block schedule is available yet.</Alert>
      )}
                          ...
                          {assignment ? (
                            <Stack spacing={0.1} sx={{ minWidth: 0 }}>
                              <Typography fontWeight={850} fontSize={{ xs: 10.5, md: 11.5 }} sx={{ color: color?.color, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                {assignment.rotationName}{assignment.override ? " ⚠" : ""}
                              </Typography>
      ...
      <Card sx={{ mb: 1.5, borderRadius: 2 }}>
        <CardContent sx={{ p: 1.5 }}>
          <Typography fontWeight={900} sx={{ mb: 1 }}>Resident Block Counts</Typography>
          <Stack spacing={0.6}>
            {activeResidents.map((resident) => (
              <Box key={resident.id} sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "190px 1fr" }, gap: 1, py: 0.6, borderBottom: "1px solid #eef2f7" }}>
                <Box><Typography fontWeight={850} fontSize={13}>{resident.displayName}</Typography><Typography fontSize={10.5} color="text.secondary">{resident.pgy}</Typography></Box>
                <Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap>
                  {Object.entries(rotationCounts[resident.id] || {}).sort((a, b) => a[0].localeCompare(b[0])).map(([rotationName, count]) => {
                    const color = rotationColor(rotationName);
                    return <Chip key={rotationName} label={`${rotationName}: ${count}`} size="small" sx={{ height: 21, fontSize: 10.5, fontWeight: 800, color: color.color, backgroundColor: color.bg, border: `1px solid ${color.border}` }} />;
                  })}
                </Stack>
              </Box>
            ))}
    ...
    <Dialog open={open} onClose={onCancel} fullWidth maxWidth="sm">
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          ...
          {eligibility === "override" && (
            <TextField required label="Override / coverage reason" value={overrideReason} onChange={(event) => setOverrideReason(event.target.value)} placeholder="Example: PGY2 unavailable; PGY3 covering 2NC" fullWidth />
          )}
