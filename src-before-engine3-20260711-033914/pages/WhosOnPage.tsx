  ...
  return (
    <Box sx={{ width: "100%", maxWidth: "none", minWidth: 0 }}>
      <Stack direction={{ xs: "row", md: "row" }} justifyContent="space-between" alignItems="center" spacing={1} sx={{ mb: 1.5 }}>
        <Box>
          <Typography variant="h4" fontWeight={900} sx={{ fontSize: { xs: 26, md: 34 }, lineHeight: 1 }}>Who&apos;s On</Typography>
          ...
          {loading ? (
            ...
            <Stack spacing={1.5}>
              <Box>
                ...
                <ResidentTable rows={callRows} onOpenResident={(id) => onOpenResidentProfile?.(id)} />
              </Box>
              ...
              {consultRows.length > 0 && (
                <Box>
                  ...
                  <ResidentTable rows={consultRows} onOpenResident={(id) => onOpenResidentProfile?.(id)} />
                </Box>
              )}
            </Stack>
          ) : mode === "all" ? (
...
function ResidentTable({ rows, onOpenResident }: {
  rows: Array<{ service: string; time: string; residentId: string; name: string; level: string; pager: string; issues: ScheduleIssue[] }>;
  ...
  return (
    <Box sx={{ overflowX: "auto" }}>
      <Box sx={{ minWidth: 680 }}>
        ...
        {rows.map((row, index) => {
          const critical = row.issues.some((issue) => issue.severity === "critical");
          const warning = row.issues.some((issue) => issue.severity === "warning");
          return (
            <Row key={`${row.service}-${index}`} columns="1.25fr .7fr 1.25fr .85fr .75fr" index={index}>
              ...
              <Cell><Button disabled={!row.residentId} onClick={() => onOpenResident(row.residentId)} sx={{ p: 0, minWidth: 0, textTransform: "none", color: row.name ? "#0f172a" : "text.secondary", fontWeight: 800 }}>{row.name || "Unassigned"}</Button>{critical && <Chip label="Issue" size="small" sx={{ ml: 0.5, height: 17, fontSize: 9, color: "#be123c" }} />}{!critical && warning && <Chip label="Warn" size="small" sx={{ ml: 0.5, height: 17, fontSize: 9, color: "#b45309" }} />}</Cell>
        ...
        >
          <Cell>
            {row.serviceProfileId ? (
              ...
              <Typography fontSize={13} fontWeight={850} color="#0f172a">
                {row.service}
              </Typography>
            )}
          </Cell>
          ...
          <Cell>
            <Chip label={row.coverage} size="small" />
          </Cell>
          <Cell>
            <Typography
...
function Row({ children, columns, header, index = 0 }: { children: React.ReactNode; columns: string; header?: boolean; index?: number }) {
  return <Box sx={{ display: "grid", gridTemplateColumns: columns, alignItems: "center", minWidth: 620, px: 1, py: header ? 0.8 : 0.65, borderBottom: "1px solid", borderColor: "#eef2f7", backgroundColor: header ? "#e2e8f0" : index % 2 ? "#f8fafc" : "white" }}>{children}</Box>;
}
...
function IssuesPanel({ issues }: { issues: ScheduleIssue[] }) { if (!issues.length) return <Alert severity="success" sx={compactAlertSx}>No resident call conflicts detected.</Alert>; return <Card sx={{ mb: 1.5 }}><CardContent sx={{ p: 1.2 }}><Typography fontWeight={900}>Schedule Issues ({issues.length})</Typography><Stack spacing={0.4} sx={{ mt: 0.5 }}>{issues.slice(0, 3).map((issue) => { const style = issueSeverityStyle(issue.severity); return <Box key={issue.id} sx={{ p: 0.6, borderRadius: 1.5, backgroundColor: style.bg, border: `1px solid ${style.border}` }}><Typography fontSize={11.5} fontWeight={900} sx={{ color: style.color }}>{issue.title}</Typography><Typography fontSize={11}>{issue.message}</Typography></Box>; })}</Stack></CardContent></Card>; }
